...
## Core: drawings 6 and 7

Drawing 21 (lamination stamping and cutting schedule) and drawing 22 (core
cutting chart) are the same core's own cutting documents, thematically part
of this same group -- see drawings 21 and 22, placed later in this register
in their own numeric position, not here, since this document's own section
order now matches the register's numeric order throughout (see "Where each
drawing appears").
...
## Insulation and assembly: drawings 11 and 12

Drawing 20 (cross-sectional drawings) shares this same section machinery and
reads as one pair with drawing 12 -- see drawing 20, placed later in this
register in its own numeric position, not here.
...
## Cross-sectional drawings: 20

Shares its section machinery with drawing 12 (see "Insulation and assembly:
drawings 11 and 12" above) -- placed here, in its own numeric position, not
grouped with 11/12 in this document's own section order, for the same reason
21 and 22 are placed after this rather than grouped with 6/7: this document's
own section order matches the register's numeric order throughout, so a
reader's position in this file always matches a reader's position in the 2D
Drawings tab.
...
## Core stamping and cutting: 21 and 22

Thematically part of the core group (see "Core: drawings 6 and 7" above) --
placed here, in numeric position, for the same reason drawing 20 is placed
where it is rather than next to drawing 12.
...
- **2D Drawings tab:** drawings 1 through 19 in ascending order, then
  drawing 20's two sheets (longitudinal, then transverse), then drawing 21,
  then drawing 22 -- the exact order this document's own sections now
  appear in, top to bottom, so a reader's position in either always matches
  the other. 23 physical sheets from 22 numbered drawings, since drawing 20
  alone produces two.
